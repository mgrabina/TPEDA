...
public class Listeners {
	...
	public static void llamadaDeCasillero(int fil, int col) {
		if (!game.getNext().esMaquina()) // Si le toca a la persona
			if (game.mover(fil, col, game.getNext(), true)) { // Si se puede
																// hacer el
																// movimiento
																// que quiere
	...
	private static void agregarFicha(int fil, int col) {
		...
		GUI.actualizarPuntajes(game.getPersona().getNombre(),
				game.getPersona().getPuntos() + game.getPersona().getPuntosT(),
				game.getMaquina().getPuntos() + game.getMaquina().getPuntosT());
	}
