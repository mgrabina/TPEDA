...
public class Go {
	...
	// devuelve la cantidad de casilleros en un territorio; si devuelve 0
	// entonces no es territorio
	...
	int[] MINIMAX(Jugador j, boolean crearDot) {
		// Hacer arbol de movimientos, usando la clase tree y move que cuando se
		// crea se asigna su heuristica.
	...
	public Integer minimax(Tree<ArrayList<Move>> tree, int depth, int currentLevel, boolean color, int alfa, int beta) {
		...
		if (currentLevel == depth) {
			int heuristicaDeLaHoja = ((ArrayList<Move>) tree.getValue())
					.get(((ArrayList<Move>) tree.getValue()).size() - 1).heuristica(tree.getValue());

			((ArrayList<Move>) tree.getValue()).get(((ArrayList<Move>) tree.getValue()).size() - 1)
					.setHeuristica(heuristicaDeLaHoja);
	...
	public int minimax(Tree<ArrayList<Move>> tree, int depth, int currentLevel, boolean color, Long time, int alfa,
		...
		while (System.currentTimeMillis() / 1000 - start < time) {
			for (Tree<ArrayList<Move>> t : tree.getChildren()) // Por cada hijo
																// hay una
																// recursiva
				minimax(t, depth, currentLevel + 1, !color, alfa, beta);
		}
